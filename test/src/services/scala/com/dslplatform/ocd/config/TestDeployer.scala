...
private[config] class TestDeployer(logger: Logger
            , testSettings: ITestSettings) extends ITestDeployer {
  ...
  val commonBuildTemplateName = s"build-common-template-${testSettings.revenj.templateName}-${testSettings.database.templateName}.xml"
  ...
  class TestSetup(testProject: ITestProject, namesAndPorts: NamesAndPorts) {
    ...
    private def copyTemplate(source: Path, target: Path) = {
      logger.trace("Copying templates...")
      logger.trace("Creating the " + source.name + " script: " + target.path)

      target.write(applyTemplates(source.string))
    }
    ...
    private def deployEclipseProject(): Unit = {
      ...
      testProject.testFiles.keys foreach { case language =>
        val projectRootForLanguage = languageRoot(language)

        language match {
          case JAVA =>
            val classpathPath = projectRootForLanguage / ".classpath"
