...
public class JsonSerializationDinamo {
    ...
    private static final JsonDeserializer<Point2D> locationDeserializer = new JsonDeserializer<Point2D>() {
        ...
        public Point2D deserialize(
                ...
                JsonProcessingException {
            final JsonNode tree = parser.getCodec().readTree(parser);
            return new Point2D.Double(tree.get("x").asDouble(), tree.get("y")
                    .asDouble());
        }
    };
    ...
    private static final JsonDeserializer<Rectangle2D> rectangleDeserializer = new JsonDeserializer<Rectangle2D>() {
        ...
        public Rectangle2D deserialize(
                ...
                JsonProcessingException {
            final JsonNode tree = parser.getCodec().readTree(parser);
            return new Rectangle2D.Double(tree.get("x").asDouble(), tree.get(
                    "y").asDouble(), tree.get("width").asDouble(), tree.get(
                    "height").asDouble());
        }
    };
...
//        System.out.println(el.getOwnerDocument().getDoctype());
        ...
        for (final Map.Entry<String, LinkedList<Node>> name_ChildrenHavingName : childrenByName
            ...
            final Object[] items = new Object[name_ChildrenHavingName
                    .getValue().size()];
        ...
        if (jsonHashMap.size() == 1
            ...
            if (name.equals(textNodeTag)) {
                return jsonHashMap.get(name);
            } else {
                ...
                if (childEntry.getKey().startsWith("@")) {
                    ...
                    if (childEntry.getKey().equals(textNodeTag)) {
                        if (childEntry.getValue() instanceof List) {
                            ...
                            final Node textNode = doc.createTextNode(childEntry
                                    .getValue().toString());
                        ...
                        if (childEntry.getValue() instanceof List) {
                            ...
                            final Node cDataNode = doc
                                    .createCDATASection(childEntry.getValue()
                                            .toString());
                        ...
                        if (childEntry.getValue() instanceof List) {
                            ...
                            final Node commentNode = doc
                                    .createComment(childEntry.getValue()
                                            .toString());
                    ...
                    final Element newElement = doc.createElement(childEntry
                            .getKey());
    ...
    private static void buildTextNodeList(
        ...
        StringBuffer singleTextNodeValue = new StringBuffer();
    ...
    private static void buildXmlFromJsonArray(
        ...
        for (final Object elementContent : elementContentList.subList(1,
            ...
            final Element newElement = doc.createElement(listHeadNode
                    .getNodeName());
    ...
    public JsonSerializationDinamo(
